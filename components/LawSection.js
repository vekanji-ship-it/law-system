...
export default function LawSection({ isPaid }) {
  ...
  const handleCategorySelect = (catId, searchTerm = '') => {
    ...
    <div>
      ...
      <div style={{ marginBottom: '12px', position: 'relative' }}>
        <input
          ...
          style={{ width: '100%', padding: '10px 40px 10px 14px', border: '1.5px solid #e2e8f0', borderRadius: '8px', fontSize: '14px', boxSizing: 'border-box' }}
        />
        {searchText && (
          <button
            onClick={() => setSearchText('')}
            style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', color: '#94a3b8', lineHeight: 1, padding: '2px' }}>
            ✕
          </button>
        )}
      </div>
      ...
      {/* 搜尋無結果提示 */}
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '48px 24px', background: 'white', borderRadius: '10px', border: '1px solid #e2e8f0', color: '#64748b' }}>
