...
export default function LawSection({ isPaid }) {
  ...
  const allArticles = [...FULL_LAW_ARTICLES, ...SUPPLEMENT_LAW_ARTICLES_1, ...SUPPLEMENT_LAW_ARTICLES_2, ...SUPPLEMENT_LAW_ARTICLES_3, ...SUPPLEMENT_LAW_ARTICLES_4, ...SUPPLEMENT_LAW_ARTICLES_5]
        ...
        {[['全部', '全部'], ['high', '★ 高頻'], ['medium', '◆ 中頻'], ['low', '◇ 一般']].map(([val, label]) => (
          <button key={val} onClick={() => setShowFreq(val)}
            style={{ padding: '4px 12px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600,
              background: showFreq === val ? (val === 'high' ? '#dc2626' : val === 'medium' ? '#d97706' : '#6b7280') : '#f1f5f9',
