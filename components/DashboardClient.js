          ...
          <div>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
              {caseCategories.map(cat => (
                <button key={cat} onClick={() => setCaseCategory(cat)} className="btn btn-sm" style={{ background: caseCategory === cat ? '#0f1f3d' : '#e2e8f0', color: caseCategory === cat ? 'white' : '#374151' }}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          <div>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
              {examCategories.map(cat => (
                <button key={cat} onClick={() => setExamCategory(cat)} className="btn btn-sm" style={{ background: examCategory === cat ? '#0f1f3d' : '#e2e8f0', color: examCategory === cat ? 'white' : '#374151' }}>
                  {cat}
                </button>
              ))}
            </div>
                  ...
                  {expandedId === q.id && (
                    q.isPaid && !isPaid ? <LockWall /> : (
                      <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ background: '#fef9ec', borderLeft: '3px solid #c9973a', padding: '10px 14px', borderRadius: '0 8px 8px 0', fontSize: '13px', color: '#92400e' }}>
                          ⚖️ 引用法條：{q.law}
                        </div>
                      </div>
                    )
              ...
              <div key={l.id} className="card">
                ...
                {expandedId === l.id && (
                  l.isPaid && !isPaid ? <LockWall /> : (
                    <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid #e2e8f0' }}>
                      <div style={{ background: '#f8fafc', borderRadius: '8px', padding: '16px', marginBottom: '16px', fontFamily: 'monospace', fontSize: '13px', lineHeight: 1.8, color: '#374151', whiteSpace: 'pre-wrap', borderLeft: '4px solid #0f1f3d' }}>
                        {l.fullText}
                      </div>
